...
{
    public static final HashMap<String, HashMap<String,Long>> hopper_counter = new HashMap<String, HashMap<String,Long>>();
    public static final HashMap<String, Long> hopper_counter_start_tick = new HashMap<String, Long>();
    public static final HashMap<String, Long> hopper_counter_start_millis = new HashMap<String, Long>();

    static {
        for (EnumDyeColor color : EnumDyeColor.values())
        {
            String col_str = color.toString();
            hopper_counter.put(col_str, new HashMap<String, Long>());
    ...
    public static void count_hopper_items(World worldIn, EnumDyeColor color, ItemStack itemstack)
    {
        ...
        {
            hopper_counter_start_tick.put(color_string, (long)worldIn.getMinecraftServer().getTickCounter());
            hopper_counter_start_millis.put(color_string, MinecraftServer.getCurrentTimeMillis());
        }
    ...
    public static void reset_hopper_counter(World worldIn, String color)
    {
        if (color == null)
        {
            for (EnumDyeColor clr : EnumDyeColor.values())
            {
                ...
                hopper_counter_start_tick.put(clr.toString(), (long)worldIn.getMinecraftServer().getTickCounter());
                hopper_counter_start_millis.put(clr.toString(), MinecraftServer.getCurrentTimeMillis());
            }
        }
        else
        {
            hopper_counter.put(color, new HashMap<String, Long>());
            hopper_counter_start_tick.put(color, (long) worldIn.getMinecraftServer().getTickCounter());
            hopper_counter_start_millis.put(color, MinecraftServer.getCurrentTimeMillis());
        }
    }
    ...
    public static List<ITextComponent> query_hopper_stats_for_color(MinecraftServer server, String color, boolean realtime, boolean brief)
    {
        ...
        if (realtime)
        {
            total_ticks = (MinecraftServer.getCurrentTimeMillis()-hopper_counter_start_millis.get(color))/50L+1L;
        }
        else
        {
