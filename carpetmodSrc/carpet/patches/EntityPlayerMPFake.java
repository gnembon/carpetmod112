...
public class EntityPlayerMPFake extends EntityPlayerMP {
    ...
    public static EntityPlayerMPFake createAstral(MinecraftServer server, EntityPlayerMP player) {
        ...
        server.getPlayerList().initializeConnectionToPlayer(new NetworkManagerFake(EnumPacketDirection.CLIENTBOUND),
                astral);
        server.getPlayerList().serverUpdateMovingPlayer(astral);
        server.getPlayerList()
                .sendPacketToAllPlayers(new SPacketEntityHeadLook(astral, (byte) (astral.rotationYawHead * 256 / 360)));
