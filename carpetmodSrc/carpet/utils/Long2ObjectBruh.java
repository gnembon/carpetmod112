...
/* This header is separate from features.h so that the compiler can
   ...
   test2 macros that may be defined in a source file before it first
...
public class Long2ObjectBruh<V> extends AbstractLong2ObjectMap<V> implements java.io.Serializable, Cloneable, Hash {
    ...
    public V remove(final Object ok) {
        ...
        int pos;
        // The starting point.
        if (((curr = key[pos = (int) HashCommon.mix((k)) & mask]) == (0))) return (this.defRetValue);
     ...
     */
    final class MapEntry implements Entry<V>, Map.Entry<Long, V> {
        ...
        public int hashCode() {
            return HashCommon.long2int(key[index]) ^ ((value[index]) == null ? 0 : (value[index]).hashCode());
        }
        ...
        public int nextEntry() {
            ...
            for (;;) {
                if (--pos < 0) {
                    ...
                    int p = (int) HashCommon.mix((k)) & mask;
                    while (!((k) == (key[p])))
                        p = (p + 1) & mask;
                    return p;
                }
    ...
    private class EntryIterator extends MapIterator implements ObjectIterator<Entry<V>> {
    ...
    private class FastEntryIterator extends MapIterator implements ObjectIterator<Entry<V>> {
    ...
    private final class MapEntrySet extends AbstractObjectSet<Entry<V>> implements FastEntrySet<V> {
     ...
     */
    public boolean trim() {
        final int l = HashCommon.arraySize(size, f);
        if (l >= n || size > HashCommon.maxFill(l, f)) return true;
        try {
            rehash(l);
    ...
    protected void rehash(final int newN) {
        ...
        for (int j = realSize(); j-- != 0;) {
            while (((key[--i]) == (0)))
                ;
            if (!((newKey[pos = (int) HashCommon.mix((key[i])) & mask]) == (0))) while (!((newKey[pos = (pos + 1) & mask]) == (0)))
                ;
            newKey[pos] = key[i];
